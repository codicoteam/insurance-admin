...
const MOCK_QUOTES: RecoveryQuote[] = [
  {
    ...
    claimId: "CLM-8821",
    recoveryAmount: 14200,
    status: "Recovered",
    leadSource: "Customer Call",
  ...
  },
  {
    ...
    claimId: "CLM-7743",
    recoveryAmount: 32600,
    status: "Approved",
    leadSource: "Email Reminder",
  ...
  },
  {
    ...
    claimId: "CLM-3312",
    recoveryAmount: 21500,
    status: "Approved",
    leadSource: "Field Agent Visit",
  ...
  },
  {
    ...
    claimId: "CLM-5567",
    recoveryAmount: 47800,
    status: "Recovered",
    leadSource: "Partner / Broker",
  ...
  },
  {
    ...
    claimId: "CLM-6634",
    recoveryAmount: 16300,
    status: "Pending",
    leadSource: "Aggregator Platform",
...
const MOCK_PIPELINE: PipelineCard[] = [
  ...
  },
  {
    id: "P-008",
    quoteId: "RQ-020",
    customerName: "Jin-Ho Lee",
    claimId: "CLM-2278",
    recoveryAmount: 61500,
    stage: "Negotiation",
    leadSource: "Email Reminder",
    agent: "Lisa Johnson",
...
const MOCK_STATS: LeadSourceStat[] = [
  {
    ...
    totalCases: 118,
    converted: 72,
    recoveredAmount: 548200,
  },
  {
    source: "Email Reminder",
    totalCases: 94,
    converted: 51,
    recoveredAmount: 412600,
  },
  {
    source: "Website",
    totalCases: 136,
    converted: 63,
    recoveredAmount: 319800,
  },
  {
    source: "Field Agent Visit",
    totalCases: 77,
    converted: 58,
    recoveredAmount: 724500,
  },
  {
    source: "Partner / Broker",
    totalCases: 52,
    converted: 44,
    recoveredAmount: 982300,
  },
  {
    source: "Aggregator Platform",
    totalCases: 89,
    converted: 37,
    recoveredAmount: 218900,
  },
];
...
const LeadBadge: React.FC<{
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full font-semibold ${colors.badge} ${
...
const BarChart: React.FC<{
  ...
  const get = (s: LeadSourceStat) => {
    if (metric === "conversionRate")
      return Math.round((s.converted / s.totalCases) * 100);
      ...
      {data.map((s, i) => {
        ...
        const label =
          metric === "conversionRate"
            ? `${val}%`
            : metric === "recoveredAmount"
              ? fmtK(val)
              : String(val);
        return (
          <div key={s.source} className="flex items-center gap-3 group">
            <span className="text-xs text-gray-500 w-36 shrink-0 truncate group-hover:text-gray-700 transition-colors">
              {s.source}
            </span>
...
const QuoteModal: React.FC<{
  onClose: () => void;
  onSave: (
    q: Omit<RecoveryQuote, "id" | "createdAt" | "customerId" | "status">,
  ) => void;
}> = ({ onClose, onSave }) => {
  ...
  const set = (k: keyof QuoteFormData, v: string) =>
    setForm((p) => ({ ...p, [k]: v }));
      ...
      <div className="bg-white border border-gray-200 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            ).map(({ label, key, type, req, colSpan }) => (
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              type="submit"
              className="flex-1 py-2.5 bg-blue-600 hover:bg-blue-700 rounded-xl text-sm font-semibold text-white transition-colors shadow-sm"
            >
              Create Quote
            </button>
          </div>
...
export default function InsuranceRecoveryDashboard() {
  ...
  const totalRecovered = useMemo(
    () =>
      quotes
        .filter((q) => q.status === "Recovered")
        .reduce((s, q) => s + q.recoveryAmount, 0),
    [quotes],
  );
  ...
  const activePipelineValue = useMemo(
    () =>
      pipeline
        ...
        .reduce((s, p) => s + p.recoveryAmount, 0),
    [pipeline],
  );
  ...
  const stats = [
    {
      label: "Total Cases",
      value: quotes.length,
      sub: `${quotes.filter((q) => q.status === "Recovered").length} recovered`,
      icon: FileText,
      color: "blue",
      trend: "+14%",
      up: true,
    ...
    },
    {
      ...
      value: fmtK(activePipelineValue),
      sub: `${pipeline.filter((p) => p.stage !== "Closed").length} active cases`,
      icon: TrendingUp,
      color: "purple",
      trend: "+9%",
      up: true,
  ...
  const filteredQuotes = useMemo(
    () =>
      quotes.filter((q) => {
        const ms =
          q.customerName.toLowerCase().includes(qSearch.toLowerCase()) ||
  ...
  const handleAdd = useCallback(
    (
      data: Omit<RecoveryQuote, "id" | "createdAt" | "customerId" | "status">,
    ) => {
  ...
  const onDrop = (e: React.DragEvent, stage: PipelineStage) => {
    ...
    setPipeline((prev) =>
      prev.map((c) => (c.id === dragCardId.current ? { ...c, stage } : c)),
    );
  ...
  return (
    <div className="flex min-h-screen bg-white">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    Insurance Recovery Management Dashboard
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div
                        className={`p-2.5 rounded-xl ${colorClasses[stat.color as keyof typeof colorClasses]}`}
                      >
                        <Icon size={20} />
                      </div>
                      <span
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <div className="relative">
                      ...
                      <select
                        value={qSource}
                        onChange={(e) =>
                          setQSource(e.target.value as RecoveryLeadSource | "")
                        }
                    ...
                    <div className="relative">
                      <select
                        value={qStatus}
                        onChange={(e) =>
                          setQStatus(e.target.value as RecoveryStatus | "")
                        }
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-2 flex-wrap items-center">
                  <div className="relative">
                    ...
                    <select
                      value={pSource}
                      onChange={(e) =>
                        setPSource(e.target.value as RecoveryLeadSource | "")
                      }
                      className="pl-8 pr-7 py-2.5 text-sm border border-gray-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 appearance-none"
                    >
                      <option value="">All Sources</option>
                  ...
                  <div className="relative">
                    <select
                      value={pStage}
                      onChange={(e) =>
                        setPStage(e.target.value as PipelineStage | "")
                      }
                      className="pl-3 pr-7 py-2.5 text-sm border border-gray-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 appearance-none"
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-7 gap-4">
                  {PIPELINE_STAGES.map((stage) => {
                    ...
                    const stageVal = cards.reduce(
                      (s, c) => s + c.recoveryAmount,
                      0,
                    );
                    const StageIcon =
                      stage === "Recovered"
                    ...
                    return (
                      ...
                      >
                        {/* Column header */}
                        <div
                          className={`rounded-xl p-4 text-white ${STAGE_CONFIG[stage].header} shadow-sm`}
                        >
                          <div className="flex items-center gap-2 mb-2">
                            <StageIcon size={16} />
                        ...
                        <div className="space-y-2 min-h-[150px]">
                          {cards.map((card) => {
                            const SourceIcon =
                              LEAD_SOURCE_ICONS[card.leadSource];
                            const sourceColors =
                              LEAD_SOURCE_COLORS[card.leadSource];
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="flex items-center gap-2 mb-2">
                                  <div
                                    className={`p-1 rounded-lg ${sourceColors.bg}`}
                                  >
                                    <SourceIcon
                                      className={sourceColors.text}
                                      size={10}
                                ...
                                <div className="mt-2 space-y-2">
                                  ...
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-xs text-gray-500 font-semibold w-7 text-right">
                                      {card.probability}%
                                    </span>
                                  </div>
                                </div>
              ...
              <div className="space-y-6">
                ...
                <div className="flex gap-1 bg-white border border-gray-100 rounded-xl p-1 w-fit shadow-sm">
                  {(
                    ...
                    ] as { key: AnalyticMetric; label: string }[]
                  ).map(({ key, label }) => (
                    <button
                      key={key}
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
                  ...
                  <div className="bg-white border border-gray-200 rounded-2xl p-6">
                    <h2 className="text-base font-bold text-gray-900 mb-4 flex items-center gap-2">
                      <Zap size={16} className="text-amber-500" /> Top
                      Performers
                    </h2>
                    <div className="space-y-3">
                      {[...MOCK_STATS]
                        .sort((a, b) => {
                          ...
                          if (metric === "conversionRate")
                            return (
                              b.converted / b.totalCases -
                              a.converted / a.totalCases
                            );
                ...
                <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
                  <div className="px-6 py-4 border-b border-gray-200">
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">
                      Comprehensive performance metrics across all acquisition
                      channels
                    </p>
                  </div>
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {MOCK_STATS.map((s) => {
                          const rate = Math.round(
                            (s.converted / s.totalCases) * 100,
                          );
                          const avg =
                            s.converted > 0
                              ? Math.round(s.recoveredAmount / s.converted)
                              : 0;
                          const Icon = LEAD_SOURCE_ICONS[s.source];
                          const colors = LEAD_SOURCE_COLORS[s.source];
                          return (
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                  {[
                    {
                      label: "Best Conv. Source",
                      value: "Partner / Broker",
                      sub: "84.6% conversion",
                      icon: Award,
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`inline-flex p-2 rounded-xl mb-3 ${card.bg}`}
                        >
                        ...
                        <p
                          className={`text-xs font-semibold mt-1.5 ${card.color}`}
                        >
                          {card.label}
                        </p>
                      </div>
            ...
            <div className="flex items-start gap-3 p-4 bg-blue-50 rounded-lg border border-blue-200">
              ...
              <p className="text-blue-900 text-sm">
                Recovery Operations Dashboard provides end-to-end visibility of
                insurance recovery cases. Track quotes through the pipeline,
                monitor recovery amounts, and analyze lead source performance.
      ...
      {showModal && (
        <QuoteModal onClose={() => setShowModal(false)} onSave={handleAdd} />
      )}
    </div>
  );
}
