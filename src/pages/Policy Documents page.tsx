...
const MOCK_DOCS: PolicyDocument[] = [
  {
    id: "DOC-001",
    policyId: "POL-001",
    ownerType: "Policyholder",
    documentType: "Policy Document",
    fileName: "policy_contract_harrington.pdf",
    fileFormat: "PDF",
  ...
  },
  {
    ...
    ownerType: "Policyholder",
    documentType: "KYC",
    fileName: "harrington_national_id.jpg",
    fileFormat: "JPG",
    uploadDate: "2024-01-10",
    expiryDate: "2029-06-01",
    status: "Verified",
    notes: "National ID – front & back",
  },
  {
    id: "DOC-003",
    policyId: "POL-002",
    ...
    documentType: "KYC",
    fileName: "caldwell_birth_certificate.pdf",
    fileFormat: "PDF",
  ...
  },
  {
    ...
    policyId: "POL-003",
    ownerType: "Asset",
    documentType: "Asset Document",
    fileName: "vehicle_registration_bmw_x5.pdf",
    fileFormat: "PDF",
    uploadDate: "2024-02-20",
    expiryDate: "2025-11-20",
    status: "Verified",
    notes: "Vehicle title document",
  },
  {
    id: "DOC-005",
    policyId: "POL-003",
    ownerType: "Asset",
    documentType: "Asset Document",
    fileName: "car_inspection_report_2024.docx",
    fileFormat: "DOCX",
  ...
  },
  {
    ...
    fileName: "claim_incident_report_pol004.pdf",
    fileFormat: "PDF",
    uploadDate: "2024-03-08",
    status: "Rejected",
    notes: "Illegible scan – resubmit required",
  },
  {
    id: "DOC-007",
    policyId: "POL-004",
    ownerType: "Policyholder",
    documentType: "KYC",
    fileName: "whitfield_passport_copy.png",
    fileFormat: "PNG",
  ...
  },
  {
    ...
    ownerType: "Claim",
    documentType: "Claim Document",
    fileName: "medical_invoice_boston_clinic.pdf",
    fileFormat: "PDF",
    uploadDate: "2024-03-20",
    status: "Pending",
    notes: "MRI claim supporting doc",
  },
  {
    id: "DOC-009",
    policyId: "POL-005",
    ownerType: "Asset",
    documentType: "Asset Document",
    fileName: "mri_equipment_appraisal.pdf",
    fileFormat: "PDF",
    uploadDate: "2024-03-22",
    expiryDate: "2026-03-22",
    status: "Verified",
    notes: "Third-party appraisal",
  },
  {
    id: "DOC-010",
    policyId: "POL-006",
    ownerType: "Policyholder",
    documentType: "Policy Document",
    fileName: "thornton_policy_renewal_2024.pdf",
    fileFormat: "PDF",
    uploadDate: "2024-04-01",
    expiryDate: "2025-04-01",
    status: "Verified",
    notes: "Annual renewal document",
  },
  {
    id: "DOC-011",
    policyId: "POL-007",
    ownerType: "Beneficiary",
    documentType: "KYC",
    fileName: "bellamy_marriage_certificate.pdf",
    fileFormat: "PDF",
    uploadDate: "2024-04-10",
    status: "Pending",
    notes: "Supporting relationship proof",
  },
  {
    id: "DOC-012",
    policyId: "POL-007",
    ownerType: "Claim",
    documentType: "Claim Document",
    fileName: "fire_damage_assessment_2024.pdf",
    fileFormat: "PDF",
    uploadDate: "2024-04-18",
    status: "Rejected",
    notes: "Missing adjuster signature",
  },
  {
    id: "DOC-013",
    policyId: "POL-008",
    ownerType: "Policyholder",
    documentType: "KYC",
    fileName: "nambiar_drivers_license.png",
    fileFormat: "PNG",
  ...
  },
  {
    ...
    ownerType: "Asset",
    documentType: "Asset Document",
    fileName: "ski_cabin_title_deed.pdf",
    fileFormat: "PDF",
    uploadDate: "2024-05-14",
    expiryDate: "2034-05-14",
    status: "Pending",
    notes: "Title deed under review",
  },
  {
    id: "DOC-015",
    policyId: "POL-010",
    ownerType: "Claim",
    documentType: "Claim Document",
    fileName: "motorcycle_accident_report.pdf",
    fileFormat: "PDF",
...
const StatusBadge: React.FC<{ status: PolicyDocument["status"] }> = ({
  status,
}) => {
  const map = {
    Pending: {
...
const OwnerBadge: React.FC<{ type: PolicyDocument["ownerType"] }> = ({
  type,
}) => {
  const map = {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border ${map[type]}`}
    >
      {ownerTypeIcon(type, "w-3 h-3")}
      {type}
    </span>
  );
};

const DocTypeBadge: React.FC<{ type: PolicyDocument["documentType"] }> = ({
  type,
}) => {
  const map = {
...
const PreviewModal: React.FC<{ doc: PolicyDocument; onClose: () => void }> = ({
  doc,
  onClose,
}) => (
      ...
      <div className="p-6 space-y-5">
        <div className="bg-gray-50 border-2 border-dashed border-gray-300 rounded-xl p-10 text-center">
          ...
          <p className="text-sm text-gray-400 mt-1">
            {doc.fileFormat} • Preview not available in demo
          </p>
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            {
              label: "Upload Date",
              value: fmtDate(doc.uploadDate),
              icon: Calendar,
            },
            {
              label: "Expiry Date",
              value: doc.expiryDate ? fmtDate(doc.expiryDate) : "N/A",
              icon: Calendar,
            },
...
const UploadModal: React.FC<{
  ...
  const base = (err?: string) =>
    `w-full px-3 py-2.5 text-sm border rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all ${err ? "border-red-400 bg-red-50" : "border-gray-200 bg-gray-50 focus:bg-white"}`;
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-4">
            ...
            <Field label="Policy ID" required error={formErrors.policyId}>
              <div className="relative">
                ...
                <input
                  ...
                  className={`${base(formErrors.policyId)} pl-9`}
                />
              </div>
            </Field>
          ...
          <Field label="File Upload" required error={formErrors.fileName}>
            <label
              className={`block border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-colors ${formErrors.fileName ? "border-red-400 bg-red-50" : "border-gray-300 bg-gray-50 hover:border-blue-400 hover:bg-blue-50"}`}
            >
              ...
              <input
                ...
                onChange={(e) =>
                  onChange("fileName", e.target.files?.[0]?.name ?? "")
                }
              />
              {formData.fileName && (
...
export default function PolicyDocumentsPage() {
  ...
  const [docTypeFilter, setDocTypeFilter] = useState<
    "All" | PolicyDocument["documentType"]
  >("All");
  const [statusFilter, setStatusFilter] = useState<
    "All" | PolicyDocument["status"]
  >("All");
  const [ownerFilter, setOwnerFilter] = useState<
    "All" | PolicyDocument["ownerType"]
  >("All");
  ...
  const totalPolicies = useMemo(
    () => new Set(docs.map((d) => d.policyId)).size,
    [docs],
  );
  ...
  const stats = [
    {
      ...
      value: docs.length,
      sub: `Across ${totalPolicies} policies`,
      icon: FileText,
      color: "blue",
    },
    {
    ...
    },
    {
      label: "Verified",
      value: verifiedCount,
      sub: `${Math.round((verifiedCount / docs.length) * 100)}% approved`,
      icon: FileCheck,
      color: "green",
    },
    {
  ...
  const actionButtons = [
    ...
    },
    {
      icon: FileText,
      label: "Policy Docs",
      onClick: () => setDocTypeFilter("Policy Document"),
      color: "indigo",
    },
    {
      icon: FileBadge,
      label: "KYC",
      onClick: () => setDocTypeFilter("KYC"),
      color: "cyan",
    },
    {
      icon: FileCheck,
      label: "Asset Docs",
      onClick: () => setDocTypeFilter("Asset Document"),
      color: "emerald",
    },
    {
      icon: FileX,
      label: "Claim Docs",
      onClick: () => setDocTypeFilter("Claim Document"),
      color: "rose",
    },
  ];
  ...
  const filtered = useMemo(() => {
    ...
    return docs.filter((d) => {
      const matchSearch =
        !q ||
        d.policyId.toLowerCase().includes(q) ||
        d.fileName.toLowerCase().includes(q) ||
        d.id.toLowerCase().includes(q);
      const matchType =
        docTypeFilter === "All" || d.documentType === docTypeFilter;
  ...
  const handleApprove = (id: string) => {
    if (window.confirm("Approve this document? This will mark it as Verified."))
      setDocs((p) =>
        p.map((d) => (d.id === id ? { ...d, status: "Verified" as const } : d)),
      );
  };
  ...
  const handleReject = (id: string) => {
    if (window.confirm("Reject this document? This will mark it as Rejected."))
      setDocs((p) =>
        p.map((d) => (d.id === id ? { ...d, status: "Rejected" as const } : d)),
      );
  };
  ...
  const validate = (): boolean => {
    ...
    if (!formData.fileName.trim())
      e.fileName = "Please select a file to upload";
    setFormErrors(e);
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    Manage, verify, and monitor all uploaded insurance documents
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="max-w-7xl mx-auto px-6 py-6">
              ...
              <div className="flex flex-col sm:flex-row gap-3 mb-4">
                ...
                <div className="relative">
                  ...
                  <select
                    value={docTypeFilter}
                    onChange={(e) => {
                      setDocTypeFilter(e.target.value as typeof docTypeFilter);
                      resetPage();
                    }}
                ...
                <div className="relative">
                  ...
                  <select
                    value={ownerFilter}
                    onChange={(e) => {
                      setOwnerFilter(e.target.value as typeof ownerFilter);
                      resetPage();
                    }}
                    className="pl-9 pr-8 py-2.5 text-sm border border-gray-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent appearance-none cursor-pointer"
                  >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
              {paginated.length === 0 ? (
                ...
                paginated.map((doc) => {
                  const OwnerIcon =
                    doc.ownerType === "Policyholder"
                      ? User
                      : doc.ownerType === "Beneficiary"
                        ? Heart
                        : doc.ownerType === "Asset"
                          ? Package
                          : Briefcase;
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-start justify-between mb-4">
                        <div
                          className={`w-12 h-12 rounded-xl bg-gradient-to-br ${ownerTypeColor(doc.ownerType)} flex items-center justify-center text-white shadow-md`}
                        >
                      ...
                      <div className="space-y-2 text-sm border-t border-gray-100 pt-3 mt-2">
                        <div className="flex items-center justify-between">
                          ...
                          <span
                            className={`px-2 py-0.5 text-xs font-bold rounded-md border ${fileFormatColor[doc.fileFormat] ?? "bg-gray-50 text-gray-700 border-gray-200"}`}
                          >
                            {doc.fileFormat}
                          </span>
                        </div>
            ...
            <div className="mt-8 flex items-start gap-3 p-4 bg-blue-50 rounded-lg border border-blue-200">
              ...
              <p className="text-blue-900 text-sm">
                This dashboard provides a comprehensive view of all
                policy-related documents. Track verification status, manage
                expirations, and take action on pending documents. Documents can
                be filtered by type, status, or owner for easy management.
              </p>
            </div>
          </div>
        </div>
